...
public class Play implements IPlugin {
    ...
    private void registerHandlers() {
        _bridge.registerHandler(k_isSignedIn, message -> {
        ...
        _bridge.registerHandler(k_signout, message -> {
            signOut();
            return "";
        });

        _bridge.registerHandler(k_showAchievements, message -> {
            showAchievements();
            return "";
        });
        ...
        _bridge.registerHandler(k_showAllLeaderboards, message -> {
            showAllLeaderboards();
            return "";
        });
    ...
    private void signInSilently() {
        ...
        _signinClient.silentSignIn().addOnCompleteListener(_activity, task -> {
    ...
    private void signOut() {
        _signinClient.signOut().addOnCompleteListener(_activity, task ->
    ...
    private void incrementAchievement(String achievementId, double percent) {
        if (!isSignedIn()) {
            return;
        }
        AchievementsClient client = getAchievementsClient();
    ...
    private void showLeaderboard(String leaderBoardId) {
        ...
        getLeaderboardsClient().getLeaderboardIntent(leaderBoardId).addOnSuccessListener(intent ->
    ...
    private void showAllLeaderboards() {
        ...
        getLeaderboardsClient().getAllLeaderboardsIntent().addOnSuccessListener(intent ->
            _activity.startActivityForResult(intent, RC_LEADER_BOARD_UI));
    }
